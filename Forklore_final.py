...
df_kamer_hoca = pd.read_csv("https://raw.githubusercontent.com/EnesAkkusci/Proj201-Datasets/main/Datasets/Finalized_KamerHoca_dataset.csv")
...
def update_results(search_value):
    ...
    if 'Recipe Name' not in df_kamer_hoca.columns or 'Region' not in df_kamer_hoca.columns:
        return [html.Div("Columns 'Recipe Name' or 'Region' not found in the dataset.")]

    filtered_df = df_kamer_hoca[df_kamer_hoca.apply(lambda row: search_value in str(row['Recipe Name']).lower() or search_value in str(row['Region']).lower(), axis=1)]
    if filtered_df.empty:
        return [html.Div("No results found.")]

    results_to_display = filtered_df.head(10)

    results = [html.Div(f"{row['Recipe Name']} - {row['Region']} - Energy: {row['Total Energy']}kcal - Protein: {row['Total Protein']}g - Carbohydrate: {row['Total Carbohydrate']}g - Sugar: {row['Total Sugar']}g - Fiber: {row['Total Fiber']}g") for _, row in results_to_display.iterrows()]
...
def update_children(year_selected,click):
    if click is None:
        ...
        co2_fig=px.histogram(co2_little,x="Country",y="2020", histfunc='avg', title=f"CO2 release of {country} and world's avg")
        ...
        if continent_row.empty:
            ...
            top_fig=px.histogram(top_five_df, x="Count",y="Ingredient", title=f"Top 5 ingredients used in {subregion}")            
...
if __name__ == '__main__':
    first.run(debug=True, host="0.0.0.0")